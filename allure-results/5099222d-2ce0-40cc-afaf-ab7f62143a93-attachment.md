...
- Name: demoblaze_e2e.spec.ts >> Demoblaze E2E Suite >> Place order successfully @regression
- Location: tests\demoblaze_e2e.spec.ts:18:7
...
Call log:
  - waiting for locator('a[onclick="addToCart(1)"]')
...
```ts
  ...
  8  |   }
  9  | 
  10 |   async addItemToCart() {
  11 |     this.page.on('dialog', dialog => dialog.accept());
> 12 |     await this.page.click('a[onclick="addToCart(1)"]');
     |                     ^ Error: page.click: Test timeout of 60000ms exceeded.
  13 |     await this.page.waitForTimeout(2000);
  14 |   }
  15 | 
  16 |   async placeOrder() {
  17 |     await this.page.click('button[data-target="#orderModal"]');
